...
export function CategoricoChart({ dados }: Props) {
    const [aba, setAba] = useState<'frequencia' | 'serie'>('frequencia');
    const frequencia = (dados.frequencia ?? []) as any[];
        ...
        <div>
            ...
            {aba === 'frequencia' && (
                <ResponsiveContainer width="100%" height={260}>
                    <BarChart layout="vertical" data={frequencia}
                        ...
                        <Tooltip
                            formatter={(v: any, _name: string, props: any) => [
                                `${v} (${props.payload?.pct}%)`, 'Ocorrências'
                            ]}
                        />
