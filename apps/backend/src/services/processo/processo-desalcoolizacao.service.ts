...
interface DesalcoolizacaoParams {
  cod_filial: number;
  data_inicial: string;
  data_final: string;
}

export async function getDesalcoolizacao(params: DesalcoolizacaoParams) {
  const { cod_filial, data_inicial, data_final } = params;

  // Query 1 — Âncora (FAT_DESALCOOLIZACAO)
  const lotes = await blabQuery<{ cod_lote_de_controle_de_qualidade: string }>(
    `SELECT DISTINCT
        DESAL.cod_lote_de_controle_de_qualidade
    FROM FAT_DESALCOOLIZACAO DESAL
    WHERE DESAL.cod_filial = ?
    AND DESAL.data_evento BETWEEN ? AND ?
    AND DESAL.D_E_L_E_T IS NULL`,
