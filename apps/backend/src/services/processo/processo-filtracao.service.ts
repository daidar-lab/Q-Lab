...
interface FiltracaoParams {
  cod_filial: number;
  data_inicial: string;
  data_final: string;
}

export async function getFiltracao(params: FiltracaoParams) {
  const { cod_filial, data_inicial, data_final } = params;

  // Query 1 — Âncora (FAT_FILTRACAO)
  const lotes = await blabQuery<{ cod_lote_de_controle_de_qualidade: string }>(
    `SELECT DISTINCT
        FIL.cod_lote_de_controle_de_qualidade
    FROM FAT_FILTRACAO FIL
    WHERE FIL.cod_filial = ?
    AND FIL.data BETWEEN ? AND ?
    AND FIL.D_E_L_E_T IS NULL`,
