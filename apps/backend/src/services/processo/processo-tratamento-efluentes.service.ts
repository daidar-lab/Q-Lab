...
interface TratamentoEfluentesParams {
  cod_filial: number;
  data_inicial: string;
  data_final: string;
}

export async function getTratamentoEfluentes(params: TratamentoEfluentesParams) {
  const { cod_filial, data_inicial, data_final } = params;

  // Query 1 — Âncora (FAT_TRATAMENTO_EFLUENTE)
  const lotes = await blabQuery<{ cod_lote_de_controle_de_qualidade: string }>(
    `SELECT DISTINCT
        ETDI.cod_lote_de_controle_de_qualidade
    FROM FAT_TRATAMENTO_EFLUENTE ETDI
    WHERE ETDI.cod_filial = ?
    AND ETDI.data_evento BETWEEN ? AND ?
    AND ETDI.D_E_L_E_T IS NULL`,
