...
interface CaptacaoParams {
  cod_filial: number;
  data_inicial: string;
  data_final: string;
}

export async function getCaptacao(params: CaptacaoParams) {
  const { cod_filial, data_inicial, data_final } = params;

  // Query 1 — Âncora (FAT_CAPTACAO)
  const lotes = await blabQuery<{ cod_lote_de_controle_de_qualidade: string }>(
    `SELECT DISTINCT
        CAPT.cod_lote_de_controle_de_qualidade
    FROM FAT_CAPTACAO CAPT
    WHERE CAPT.cod_filial = ?
    AND CAPT.data_evento BETWEEN ? AND ?
    AND CAPT.D_E_L_E_T IS NULL`,
  ...
  const resultados = await blabQuery<any>(
    `SELECT
        ...
        cod_ponto_de_coleta, abreviatura, descricao
