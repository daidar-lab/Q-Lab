...
interface FermentacaoParams {
  cod_filial: number;
  data_inicial: string;
  data_final: string;
  fermentadores?: number[];
}

export async function getFermentacao(params: FermentacaoParams) {
  const { cod_filial, data_inicial, data_final, fermentadores } = params;

  // Query 1 — Âncora (FAT_FERMENTACAO)
  let sql = `SELECT DISTINCT
      FER.cod_lote_de_controle_de_qualidade
  FROM FAT_FERMENTACAO FER
  ...
  if (fermentadores && fermentadores.length > 0) {
    const placeholders = fermentadores.map(() => '?').join(', ');
    sql += ` AND FER.cod_bem IN (${placeholders})`;
    queryParams.push(...fermentadores);
  }
